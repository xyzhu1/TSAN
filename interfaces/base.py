...
from model import network, tbsrn, tsrn, edsr, srcnn, srresnet, crnn,moran,recognizer,vdsr,rdn,lapsrn,han,rcan
...
class TextBase(object):
    ...
    def generator_init(self):
        cfg = self.config.TRAIN
        if self.args.arch == 'network':
            model = network.NETWORK(scale_factor=self.scale_factor, width=cfg.width, height=cfg.height,
    ...
    def save_everyepoch(self, netG, epoch, iters, best_acc_dict, best_model_info, is_best, converge_list, exp_name):
        # ckpt_path = os.path.join('checkpoint', exp_name, self.vis_dir)
        ckpt_path = os.path.join('checkpoint', exp_name)
        if not os.path.exists(ckpt_path):
            os.mkdir(ckpt_path)
        ...
        torch.save(save_dict, os.path.join(ckpt_path, 'epoch%s.pth'%epoch))
        # '/img%s.png'% i
